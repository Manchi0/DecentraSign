...
entry fun create_private_contract_entry(
    ...
    transfer::transfer(
        create_private_contract(name, ctx.sender(), party2, encryption_key_id, ctx), 
        ctx.sender()
    );
}
...
public fun add(allowlist: &mut Allowlist, cap: &Cap, account: address) {
    ...
    if (allowlist.is_private && allowlist.contract_type == b"private_two_party") {
...
public fun get_contract_info(allowlist: &Allowlist): (String, bool, String, vector<address>) {
    (allowlist.name, allowlist.is_private, allowlist.contract_type, allowlist.list)
}
