...
export const downloadAndDecrypt = async (
  ...
  for (let i = 0; i < validDownloads.length; i += 10) {
    ...
    console.log('Processing batch:', { batchIndex: i, ids });
    ...
    try {
      ...
      const errorMsg =
        err instanceof NoAccessError
          ? 'No access to decryption keys - user not in allowlist'
  ...
  for (const encryptedData of validDownloads) {
    ...
    try {
      ...
      // Since you guarantee using PDFs, always set PDF MIME type
