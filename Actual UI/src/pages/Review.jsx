...
function Review() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('contractTerms')
    if (!stored) {
      navigate('/upload')
      return
    }
        ...
        {!showSuccess ? (
          ...
          >
            ...
            >
              ...
              <p className="review-subtitle">Review and edit your contract details before finalizing</p>
            </motion.div>
                ...
                {editableData.parties.map((party, idx) => (
                  <div key={idx} className="field-card">
                    <div className="field-header">
                      <span className="field-label">{party.role || 'Party'}</span>
                      {editingField?.section === 'parties' && editingField?.index === idx ? (
                        ...
                        <button onClick={() => handleEdit('parties', idx)} className="action-btn edit-btn">
                    ...
                    {editingField?.section === 'parties' && editingField?.index === idx ? (
                      <div className="field-inputs">
                        <input
                          type="text"
                          value={party.name}
                          onChange={(e) => handleFieldChange('parties', idx, 'name', e.target.value)}
                        ...
                        <input
                          type="email"
                          value={party.email || ''}
                          onChange={(e) => handleFieldChange('parties', idx, 'email', e.target.value)}
                          className="field-input"
                          placeholder="Email"
                        />
                      </div>
                      ...
                      <div className="field-display">
                        <p className="field-value"><strong>Name:</strong> {party.name}</p>
                        <p className="field-value"><strong>Email:</strong> {party.email || 'N/A'}</p>
                      </div>
                    )}
                  </div>
                ))}
                ...
                {editableData.payments.map((payment, idx) => (
                  <div key={idx} className="field-card payment-card">
                    <div className="field-header">
                      ...
                      {editingField?.section === 'payments' && editingField?.index === idx ? (
                        <div className="edit-actions">
                          <button onClick={handleSave} className="action-btn save-btn">
                            <CheckIcon className="icon" />
                          </button>
                        ...
                        <button onClick={() => handleEdit('payments', idx)} className="action-btn edit-btn">
                          <PencilIcon className="icon" />
                        </button>
                      )}
                    </div>
                    {editingField?.section === 'payments' && editingField?.index === idx ? (
                      <div className="field-inputs">
                        <input
                          ...
                          onChange={(e) => handleFieldChange('payments', idx, 'amount', parseFloat(e.target.value))}
                          className="field-input"
                          placeholder="Amount"
                        />
                        <input
                          ...
                          onChange={(e) => handleFieldChange('payments', idx, 'currency', e.target.value)}
                          className="field-input"
                      ...
                      <div className="field-display">
                        ...
                        <p className="field-description">{payment.description}</p>
                      </div>
                    )}
                  </div>
                ))}
                ...
                {editableData.deadlines.map((deadline, idx) => (
                  <div key={idx} className="field-card deadline-card">
                    <div className="field-header">
                      ...
                      {editingField?.section === 'deadlines' && editingField?.index === idx ? (
                        <div className="edit-actions">
                          <button onClick={handleSave} className="action-btn save-btn">
                            <CheckIcon className="icon" />
                          </button>
                        ...
                        <button onClick={() => handleEdit('deadlines', idx)} className="action-btn edit-btn">
                          <PencilIcon className="icon" />
                        </button>
                      )}
                    </div>
                    {editingField?.section === 'deadlines' && editingField?.index === idx ? (
                      <div className="field-inputs">
                        <input
                          type="date"
                          value={deadline.date}
                          onChange={(e) => handleFieldChange('deadlines', idx, 'date', e.target.value)}
                          className="field-input"
                        />
                        <input
                          type="number"
                          value={deadline.penalty || 0}
                          onChange={(e) => handleFieldChange('deadlines', idx, 'penalty', parseFloat(e.target.value))}
                          className="field-input"
                      ...
                      <div className="field-display">
                        <p className="field-value"><strong>Date:</strong> {deadline.date}</p>
                        <p className="field-value"><strong>Penalty:</strong> ${deadline.penalty} {deadline.penaltyUnit}</p>
                      </div>
                    )}
                  </div>
                ))}
            ...
            >
              ...
              <div className="wallet-input-container">
                <input
                  ...
                  maxLength={66}
                />
                <p className="wallet-hint">Enter the Sui blockchain wallet address for the recipient</p>
              </div>
            </motion.div>
          ...
          >
            ...
            <motion.p
              ...
              transition={{ delay: 0.5 }}
            >
              Your smart contract has been successfully created and is ready for deployment.
            </motion.p>
            <motion.div
            ...
            >
              <div className="summary-item">
                ...
                <span className="summary-value">{finalContract?.parties.length}</span>
              </div>
              <div className="summary-item">
                <span className="summary-label">Payments:</span>
                <span className="summary-value">{finalContract?.payments.length}</span>
              </div>
              <div className="summary-item">
                <span className="summary-label">Deadlines:</span>
                <span className="summary-value">{finalContract?.deadlines.length}</span>
              </div>
            </motion.div>
            <motion.div
