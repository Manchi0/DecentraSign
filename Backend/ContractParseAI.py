...
**CRITICAL INSTRUCTIONS FOR WALLET ADDRESSES:**
- ONLY extract wallet addresses if they are EXPLICITLY written in the contract text
- Valid Sui wallet addresses start with "0x" followed by 64 hexadecimal characters (total 66 characters)
- DO NOT generate, invent, or guess wallet addresses
- If no wallet address is found for a party, set "address" to null
- DO NOT use email addresses, names, or other data as wallet addresses
...
2. For each distinct pattern found, extract the metadata components EXACTLY as written.
3. If any specific field cannot be found, set it to null (not "Unknown").
4. Be extremely careful with wallet addresses - accuracy is critical.
...
{{
  ...
  "parties": {{
    "client": {{
      ...
      "address": "string (0x... format ONLY if explicitly in contract) or null",
      "role": "string"
    }},
    "contractor": {{
      "name": "string",
...
IMPORTANT:
1. Return actual numbers (not strings) for all numeric fields like upfront, completion, total, late_fee_per_day, max_penalty.
2. NEVER generate or guess wallet addresses - only extract if explicitly present in the text."""
